...
constexpr std::array<T, Count> mapIndices(M &&m, std::index_sequence<I...>) {
    std::array<T, Count> result{m.template operator()<I>()...};
    return result;
}
...
int bucket_fps_kdtree(const float *raw_data, size_t n_points, size_t dim,
    ...
    auto func_arr =
        map<KDTreeFuncType, max_dim + 1>(kdtree_func_helper<float>{});
    func_arr[dim](raw_data, n_points, dim, n_samples, start_idx,
...
int bucket_fps_kdline(const float *raw_data, size_t n_points, size_t dim,
    ...
    auto func_arr =
        map<KDLineFuncType, max_dim + 1>(kdline_func_helper<float>{});
    func_arr[dim](raw_data, n_points, dim, n_samples, start_idx, height,
                  sampled_point_indices);
    return 0;
}
}
